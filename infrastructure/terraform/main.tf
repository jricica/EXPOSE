...
module "rds" {
  ...
  # Restrict DB access to the application SG and any admin CIDRs
  # Note: do not reference module.ecs here because the ecs module
  # may be absent in this workspace. Provide the list of allowed
  # SGs via the variable `db_allowed_security_group_ids` instead.
...
/*
  ...
  directories are not present in the repository. They can be restored
  later by uncommenting these blocks when the corresponding modules exist.
